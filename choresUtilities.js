...
module.exports = {
     ...
     /**
      * Removes chores from a users JSON
      * @param {string} userId
      * @param {Array} chores array of chore ids to be removed
      * @param {JSON} choresJSON the JSON object containing the chores
      */
     async removeChores(userId, chores, choresJSON) {
          chores = chores.map((c) => parseInt(c) - 1);
          choresJSON["chores"] = choresJSON["chores"].filter((c) => !chores.includes(c.id));
          for (let i = 0; i < choresJSON["chores"].length; i++) {
               choresJSON["chores"][i].id = i;
          }
          const data = JSON.stringify(choresJSON);
          const choresPath = path.join(__dirname, `/users/${userId}.json`);
          try {
